function y = findcompV2(database,P,varargin)%%funzione per la minimizzazione di gibbs in condizioni adiabatiche, viene applicato il vincolo di entalpia
if numel(varargin) < 2
...
while DT > 0,001;
        if Niter == 20
            ...
            DHfin = [];
            for i = 1 : numel(ntot)
                DHfin(end+1) = COMPeq{i}.intgH(database,T_0);
            end
            heat = ntot*DHfin'-n0*DHin';
            y = [Neq,T_0,heat];
            for i=1:numel(Neq)
            ...
            if T_0 - vettore_temp(end) > 20
                if T_0 - vettore_temp(end-1) < 10 && vettore_temp(end) - vettore_temp...
                    ...
                    T_0 = (T_0+vettore_temp(end))/2;
                     ...
                     Neq = minG3(database,P,active_species);
                     Neq = Neq(1:end-1);
                     vettore_iter(end+1) = Niter;
                     vettore_temp(end+1) = T_0;
                     DHfin = [];
    ...
    Neq = minG3(database,P,active_species); % composizione all'equilibrio a T_0
    Neq = Neq(1:end-1);
    ntot = [Neq,nInert];
