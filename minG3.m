function x = minG3(database,P,varargin)% funzione di minimizzazione dell'energia libera di gibbs di una miscela di composti, in condizioni isotermiche
if numel(varargin) < 2
...
Rem = []; %vettore massa reagenti
DHre = []; %vettore entalpia reagenti
DHfin = []; %vettore entalpia prodotti
for i = 1 : numel(varargin)
    Aeq(:,i) = varargin{i}.Comp;%matrice delle composizioni di ogni specie che compare nell'equilibrio
    if varargin{i}.MassFlux > 0
        Rem(end+1) = varargin{i}.MassFlux;
        DHre(end+1) = varargin{i}.intgH(database,T);
        DHfin(end+1) = varargin{i}.intgH(database,T);
    else
        DHfin(end+1) = varargin{i}.intgH(database,T);
    end
end
